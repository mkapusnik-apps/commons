...
- Accept a hosted CI evidence packet only when `devops` reports the authoritative base-branch requirements, classifies every consulted source as `available`, `authoritatively empty`, or `inaccessible`, and ties all checks, statuses, workflow runs, and jobs to the delegated expected pull request head SHA. Require `devops` to report that delegated expected SHA, the initially observed pull request head, and the final observed head. Independently compare all three values to the SHA in the handoff; any mismatch invalidates evidence affected by the differing state and is a blocking stale-state conclusion until every impacted review, evidence item, and acceptance criterion is revalidated.
- Accept `available` only for complete evidence. For every source whose API paginates, require pagination metadata demonstrating that all pages were retrieved. Treat a partial or failed page, truncation, or unproven pagination completeness as `inaccessible` limited evidence and block when the missing data can affect requirements, an applicable hosted gate, or exact-head results.
- Accept a classic branch-protection `404 Not Found` as `authoritatively empty` only when separate successful evidence establishes that the inspecting identity has Administration access to the repository. Otherwise treat it as `inaccessible` and blocking. Apply the same cautious endpoint-specific handling to ambiguous forbidden, not-found, and access-limited responses unless independent authorization and endpoint semantics prove absence.
...
- Treat screenshot matrices as wireframe-based by default: require exactly one representative screenshot per wireframe, and do not multiply screenshots by every state, viewport, or accessibility profile unless the user or an approved product or UX specification explicitly requests exhaustive evidence. State variants remain documented in specifications and wireframes.
...
- When delegating screenshot capture to `developer`, include the complete wireframe-based `ux` screenshot matrix: exactly one representative entry per wireframe by default, with stable screen identifier, route or workflow, representative application state and setup data, viewport, expected visible behavior, destination path, and implementation state to identify in the result. Do not request every state, viewport, or accessibility profile unless the user or an approved product or UX specification explicitly requests exhaustive evidence; state variants remain documented in specifications and wireframes.
