...
- `vN.0` is a fixed reference to the first release of major `vN` and does not move.
- `v1` is a floating major tag that points to the latest compatible v1 release.
- An operator uses the manual next-major workflow when a new compatibility line is necessary.
...
The repository's patch, manual-major, and compatibility contract is defined in [semantic-releases.md](semantic-releases.md). Consumers may use `v1`, fixed `v1.0`, or immutable `v1.0.0`.
